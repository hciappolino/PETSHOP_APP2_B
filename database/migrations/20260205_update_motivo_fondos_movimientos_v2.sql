-- Update fondos_movimientos.motivo constraint to include caja-specific reasons (v2)

-- Drop known constraint name if present
ALTER TABLE fondos_movimientos
    DROP CONSTRAINT IF EXISTS fondos_movimientos_motivo_check;

-- Drop any other motivo check constraint (unknown name)
DO $$
DECLARE
...
-- Recreate with extended values (only if not present)
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
